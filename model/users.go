...
func (r *userR) GetTasks() TaskSlice {
	if r == nil {
		return nil
	}
	return r.Tasks
...
var (
	...
	userColumnsWithoutDefault = []string{"id", "name", "email", "password"}
...
func (userL) LoadTasks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`tasks`),
		qm.WhereIn(`tasks.user_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
